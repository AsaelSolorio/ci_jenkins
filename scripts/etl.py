    ...
    try:
        ...
        while True:
            ...
            if isinstance(row[4], datetime):
                row[4] = row[4].strftime('%Y-%m-%d %H:%M:%S')  # Format the datetime if exists
...
def transform_data(data):
    ...
    for row in data:
        transformed.append({
            ...
            "recorded_at": row[4].strftime("%Y-%m-%d %H:%M:%S") if isinstance(row[4], datetime) else row[4]
        })
    return transformed
...
def load_data(transformed):
    ...
    minio_client = Minio(
        "localhost:9000",
        access_key="eGuTpLjMRTZnZimN",
        secret_key="5MovpyQ9srGkP3k6dDXfJyjOVuvTgyA3",
        secure=False
    )

    minio_client.fput_object(
        "data-lake-test", filename, filename
    )
